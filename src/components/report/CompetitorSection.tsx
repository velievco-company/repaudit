...
interface Props { competitors: AuditResponse['competitors']; company: string; lang: AppLanguage; }

export default function CompetitorSection({ competitors, company, lang }: Props) {
  const avg = competitors.data.reduce((a, b) => a + b.mentions, 0) / Math.max(competitors.data.length, 1);
  const chartData = [
    { name: company, mentions: avg, isCompany: true },

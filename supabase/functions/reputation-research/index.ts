...
const Errors = {
  unauthorized: () => new Response(JSON.stringify({ error: "Unauthorized" }),                           { status: 401, headers: { ...corsHeaders, "Content-Type": "application/json" } }),
  badRequest:   (m: string) => new Response(JSON.stringify({ error: m }),                               { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }),
  rateLimited:  () => new Response(JSON.stringify({ error: "Too many requests. Try again later." }),    { status: 429, headers: { ...corsHeaders, "Content-Type": "application/json" } }),
  internal:     () => new Response(JSON.stringify({ error: "Analysis failed. Please try again." }),     { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }),
};
...
function validateInput(body: any): string | null {
  ...
  if (body.timeRange && !["3","6","12","24"].includes(body.timeRange))  return "invalid timeRange";
...
async function verifyAuth(req: Request): Promise<boolean> {
  ...
  if (apikey === supabaseAnon) return true;
  try {
    const client = createClient(Deno.env.get("SUPABASE_URL")!, supabaseAnon);
    const { data: { user }, error } = await client.auth.getUser(apikey);
    return !!user && !error;
  } catch { return false; }
}
...
async function tavilySearch(query: string, apiKey: string, maxResults = 6): Promise<string> {
  try {
    ...
    const timeout = setTimeout(() => controller.abort(), 8000); // 8s per search
...
async function gatherRealData(company: string, website: string, country: string, industry: string, apiKey: string): Promise<string> {
  ...
  // 4 searches instead of 6 — faster, stays under timeout
...
serve(async (req) => {
  ...
  // We use TransformStream to keep connection alive during long processing
  ...
  // Process in background — keeps connection alive
  (async () => {
    try {
      // Step 1: Web search
      ...
      const depthInstruction =
        depth === "deep"  ? "Extremely thorough analysis. Maximum detail." :
...
CRITICAL: Base analysis ONLY on the real web data provided. Do NOT invent facts.
If something is not in the data, note it as "not found in available sources".
You MUST use the deliver_audit tool.`;
...
=== END ===`;
      ...
      const aiResponse = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          tools: [{
            ...
            function: {
              name: "deliver_audit",
              description: "Deliver complete reputation audit",
              parameters: {
                type: "object",
      ...
      if (!aiResponse.ok) {
        ...
        await write(JSON.stringify({ error: status === 429 ? "Rate limit exceeded" : "Analysis failed" }));
        await writer.close();
        return;
      }
